...
from src.config import (
    ASK_PRIORITY, DELIVER_TURN_THRESHOLD, EXPLORE_TERM_THRESHOLD,
...
class ConversationState:
    ...
    def query_text(self) -> str:
        ...
        """
        if self.leaky_evidence or (
            not self.message_turns and not self.need.ledger and not self.constraint_phrases
        ):
    ...
    def retrieval_query(self) -> str:
        """Use raw history on the scored retrieval path; keep structured projection for state APIs."""
        return " ".join(self.all_text)
...
def next_ask(state: ConversationState, use_info_gain: bool, info_gain_mode: str) -> str | None:
    ...
    In ``ask`` mode the information-gain selector controls the evaluator-facing action.  In the
    benchmark-safe ``display`` mode it may improve the prose, but the measured legacy ask order is
    preserved in ``ask_attribute``.
    """
    if state.conv_state == "DELIVER":
        return None

    supported = {"budget", "feature", "material", "color", "style", "size", "use_case"}
    if use_info_gain and info_gain_mode == "ask" and state.ig_attr in supported:
