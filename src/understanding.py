...
class NeedModel:
    ...
    # Color is normally multi-valued ("red or blue"), while an explicit repair still emits SET.
    EXCLUSIVE_SLOTS = frozenset({"category", "brand", "size", "budget"})
    ...
    def revise(self, new: list[Constraint], is_override: bool = False) -> None:
        ...
        for c in new:
            ...
            if ((is_override or c.span is not None) and c.slot == "category" and c.polarity > 0
        ...
        for event in out:
            ...
            if event.polarity < 0:
                ...
                event.operation = "ADD"
                  ...
                  or (event.slot == "color" and previous is None)):
                event.operation = "SET"
            else:
                event.operation = "ADD"
...
def apply_category_gate(
    ...
    """Stably demote confidently wrong-category candidates without dropping them."""
    ...
    for asin in candidates:
        ...
        (demote if candidate_category and candidate_category != need_category else keep).append(asin)
    return keep + demote
